...
url = "http://127.0.0.1/ma-selection/index.php"

visited_urls = set()
#visited_urls.add("http://127.0.0.1/animation-flash/index.shtml")
		...
		for tr in tr_elements:
			...
			if len(td_elements) >= 2:
				...
				if img_element and span_element:
					...
					link = ""
					hrefs = td_elements[1].find_all('a')
					for link in hrefs:
						if link.find('img'):
							href = link
					onclick_value = ""
...
def generate_sql_file(data):
	...
	for entry in data:
		sql = f"INSERT INTO test (title, description, link, playability_info, graphism_info, interest_info, width, height, img, img2, category) VALUES "
